...
use super::{BitrotWriterWrapper, Erasure};
use bytes::Bytes;
...
impl Default for ParallelEncoderConfig {
    fn default() -> Self {
        ...
        let parallel_degree = (cpu_count / 2).clamp(2, 8); // 使用一半的CPU核心，最少2个，最多8个
...
impl ParallelErasureEncoder {
        ...
        R: AsyncRead + Send + Sync + Unpin + 'static,
    {
        ...
        let (encoded_tx, encoded_rx) = mpsc::channel::<EncodedSegment>(self.config.channel_buffer);
        ...
        let encoder_task: tokio::task::JoinHandle<std::io::Result<()>> = tokio::spawn(async move {
            ...
            loop {
                match futures.next().await {
                    Some(Err(e)) => warn!("Encoding task failed: {}", e),
                    Some(Ok(_)) => continue,
                    None => break,
                }
            }
        ...
        // 等待所有任务完成
        let (reader_result, encoder_result, write_result) = tokio::join!(reader_task, encoder_task, write_task);
    ...
    async fn encode_segment(encoder: Arc<Erasure>, segment: SegmentInfo) -> std::io::Result<EncodedSegment> {
        ...
        // 在阻塞任务中执行CPU密集型的编码操作
        let result = tokio::task::spawn_blocking(move || encoder.encode_data(&segment.data)).await?;
    ...
    async fn write_encoded_segments(
        ...
        while let Some(segment) = encoded_rx.recv().await {
            ...
            while let Some(segment) = pending_segments.remove(&next_index) {
                ...
                for (i, _shard) in segment.shards.iter().enumerate() {
                    if let Some(Some(_writer)) = writers.get_mut(i) {
                        let _shard = _shard.clone();
        ...
        if !pending_segments.is_empty() {
            ...
            return Err(std::io::Error::other("Not all segments were written"));
        }
...
mod tests {
    ...
    async fn test_parallel_encoding() {
        ...
        // 执行编码 - 由于所有writers都是None，这应该会失败
        // 但这是预期的行为，因为测试环境没有实际的写入器
        let result = encoder.encode_parallel(reader, &mut writers, 4).await;

        // 验证结果 - 由于没有可用的写入器，应该返回错误
        assert!(result.is_err());

        // 验证错误类型是正确的
        if let Err(e) = result {
            let error_msg = e.to_string();
            assert!(error_msg.contains("Write quorum not met") || error_msg.contains("No available writers"));
        }
    }
}
