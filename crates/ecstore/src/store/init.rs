...
mod tests {
    ...
    #[tokio::test(flavor = "multi_thread", worker_threads = 2)]
    #[serial_test::serial(storage_class_env)]
    async fn decommission_entry_retries_source_changed_without_canceling_other_bucket() {
        let temp_dir = tempfile::tempdir().expect("create decommission retry store dir");
        let (_ctx, store, shutdown) =
            without_storage_class_env(build_isolated_test_store(temp_dir.path(), "decommission-entry-retry", &[4, 4])).await;
        crate::bucket::metadata_sys::init_bucket_metadata_sys(store.clone(), Vec::new()).await;
        ...
        let _mutation_guard =
            crate::core::pools::DecommissionCleanupMutationGuard::install(Arc::new(move |bucket, object, attempt| {
        ...
        let _fault_guard =
            crate::core::pools::DecommissionTestFaultGuard::install(Arc::new(move |stage, bucket, object, attempt| {
    ...
    #[tokio::test(flavor = "multi_thread", worker_threads = 2)]
    #[serial_test::serial(storage_class_env)]
    async fn decommission_entry_exhausted_source_changed_retains_source_and_records_failure() {
        let temp_dir = tempfile::tempdir().expect("create decommission exhaustion store dir");
        let (_ctx, store, shutdown) =
            without_storage_class_env(build_isolated_test_store(temp_dir.path(), "decommission-entry-exhaustion", &[4, 4])).await;
        crate::bucket::metadata_sys::init_bucket_metadata_sys(store.clone(), Vec::new()).await;
    ...
    #[tokio::test(flavor = "multi_thread", worker_threads = 2)]
    #[serial_test::serial(storage_class_env)]
    async fn decommission_entry_delete_marker_copy_retries_real_path() {
        let temp_dir = tempfile::tempdir().expect("create delete marker retry store dir");
        let (_ctx, store, shutdown) =
            without_storage_class_env(build_isolated_test_store(temp_dir.path(), "decommission-delete-marker-retry", &[4, 4]))
                .await;
        crate::bucket::metadata_sys::init_bucket_metadata_sys(store.clone(), Vec::new()).await;
    ...
    #[tokio::test(flavor = "multi_thread", worker_threads = 2)]
    #[serial_test::serial(storage_class_env)]
    async fn decommission_entry_tiered_copy_retries_real_path() {
        let temp_dir = tempfile::tempdir().expect("create tiered retry store dir");
        let (ctx, store, shutdown) =
            without_storage_class_env(build_isolated_test_store(temp_dir.path(), "decommission-tiered-retry", &[4, 4])).await;
        crate::bucket::metadata_sys::init_bucket_metadata_sys(store.clone(), Vec::new()).await;
