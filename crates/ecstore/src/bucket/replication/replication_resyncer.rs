...
async fn replicate_force_delete_to_targets<S: ReplicationStorage>(dobj: &DeletedObjectReplicationInfo, storage: Arc<S>) {
    ...
    let legacy_target_arns = if dobj.delete_object.force_delete_target_arns.is_empty() && dobj.target_arn.is_empty() {
    ...
    } else if !dobj.delete_object.force_delete_target_arns.is_empty() {
        dobj.delete_object.force_delete_target_arns.clone()
    } else {
        legacy_target_arns
    };
