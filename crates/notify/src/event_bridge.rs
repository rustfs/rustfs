...
#[cfg(test)]
mod tests {
    use super::{LiveEventHistory, NotifyEventBridge};
    use crate::{Event, integration::NotificationMetrics, notifier::EventNotifier};
    use rustfs_s3_common::EventName;
    use std::sync::Arc;
    use tokio::sync::{RwLock, broadcast};

    fn build_bridge() -> NotifyEventBridge {
        ...
        NotifyEventBridge::new(notifier, live_event_sender, Arc::new(RwLock::new(LiveEventHistory::default())))
    }

    #[tokio::test]
    async fn event_bridge_reports_live_listener_subscription_state() {
        let bridge = build_bridge();
        assert!(!bridge.has_live_listeners());

        let _rx = bridge.subscribe_live_events();
        assert!(bridge.has_live_listeners());
    }

    #[tokio::test]
    async fn event_bridge_records_recent_live_events() {
        let bridge = build_bridge();
        let event = Arc::new(Event::new_test_event("bucket", "one", EventName::ObjectCreatedPut));

        bridge.send_event(event).await;

        let batch = bridge.recent_live_events_since(0, 16).await;
