...
async fn test_namespace_lock_with_clients() {
    let clients = vec![
        Some(ClientFactory::create_local()),
        Some(ClientFactory::create_local()),
        Some(ClientFactory::create_local()),
    ];
...
async fn test_namespace_lock_get_write_lock_local() {
    ...
    let guard = lock.get_write_lock(resource.clone(), "owner1", Duration::from_secs(5))
        .await
        .unwrap();
...
async fn test_namespace_lock_get_read_lock_local() {
    ...
    let guard = lock.get_read_lock(resource.clone(), "owner1", Duration::from_secs(5))
        .await
        .unwrap();
...
async fn test_namespace_lock_guard_release() {
    ...
    let mut guard = lock.get_write_lock(resource, "owner1", Duration::from_secs(5))
        .await
        .unwrap();
...
async fn test_namespace_lock_wrapper() {
    ...
    let guard = wrapper.get_write_lock(Duration::from_secs(5))
        .await
        .unwrap();
