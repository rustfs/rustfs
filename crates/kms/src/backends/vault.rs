...
mod tests {
    ...
    #[tokio::test]
    async fn test_vault_namespace_is_sent_to_request() {
        ...
        let client = VaultKmsClient::new(config).await.expect("client");
        ...
        assert!(
            ...
            "Vault requests must carry the namespace in X-Vault-Namespace or as the first path segment after /v1; got request line {:?}, header {:?}",
