...
impl CheckpointManager {
    ...
    pub async fn save_checkpoint(&self, progress: &ScanProgress) -> Result<()> {
        ...
        // 原子性替换主文件
        tokio::fs::rename(&self.temp_file, &self.checkpoint_file).await
            .map_err(|e| Error::IO(format!("替换检查点文件失败: {}", e)))?;
        
        // 更新最后保存时间
        *self.last_save.write().await = now;
        
        debug!("已保存扫描进度到 {:?}, 周期: {}, 磁盘索引: {}", 
               self.checkpoint_file, 
               checkpoint_data.progress.current_cycle,
               checkpoint_data.progress.current_disk_index);
    ...
    pub async fn load_checkpoint(&self) -> Result<Option<ScanProgress>> {
        ...
        match self.load_checkpoint_from_file(&self.checkpoint_file).await {
            Ok(checkpoint) => {
                info!("从主文件恢复扫描进度: 周期={}, 磁盘索引={}, 最后扫描key={:?}", 
                      checkpoint.current_cycle, 
                      checkpoint.current_disk_index,
                      checkpoint.last_scan_key);
                Ok(Some(checkpoint))
            },
            Err(e) => {
                warn!("主检查点文件损坏或不存在: {}", e);
    ...
    async fn load_checkpoint_from_file(&self, file_path: &Path) -> Result<ScanProgress> {
        ...
        let checkpoint_data: CheckpointData = serde_json::from_str(&content)
            .map_err(|e| Error::Serialization(format!("反序列化检查点失败: {}", e)))?;
    ...
    fn validate_checkpoint(&self, checkpoint: &CheckpointData) -> Result<()> {
        ...
        if checkpoint.node_id != self.node_id {
            return Err(Error::InvalidCheckpoint(
                format!("检查点节点ID不匹配: 期望 {}, 实际 {}", self.node_id, checkpoint.node_id)
            ));
        }
    ...
    pub async fn get_checkpoint_info(&self) -> Result<Option<CheckpointInfo>> {
        ...
        let metadata = tokio::fs::metadata(&self.checkpoint_file).await
            .map_err(|e| Error::IO(format!("获取检查点文件元数据失败: {}", e)))?;

        let content = tokio::fs::read_to_string(&self.checkpoint_file).await
            .map_err(|e| Error::IO(format!("读取检查点文件失败: {}", e)))?;

        let checkpoint_data: CheckpointData = serde_json::from_str(&content)
            .map_err(|e| Error::Serialization(format!("解析检查点失败: {}", e)))?;
