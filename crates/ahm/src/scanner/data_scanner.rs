...
impl RealtimeUsageCache {
    ...
    fn build_usage_info(&self, bucket_names: &[String]) -> DataUsageInfo {
        let mut info = DataUsageInfo::default();
        info.buckets_count = bucket_names.len() as u64;
...
impl Scanner {
    ...
    fn acquire_heal_manager(&self) -> Option<Arc<HealManager>> {
        ...
        get_heal_manager().map(|global| Arc::clone(global))
    }
    ...
    async fn collect_bucket_records_from_store(
        ...
        for object in list_result.objects {
            let usage = local_scan::LocalObjectUsage {
                ...
                last_modified_ns: object.mod_time.map(|ts| ts.unix_timestamp_nanos()).map(|ns| ns as i128),
                versions_count: 1,
    ...
    async fn collect_bucket_usage_from_ecstore(
            ...
            .await
        {
            Ok(result) => {
                ...
                usage.size = bucket_size;
                usage.objects_count = object_count;
                usage.versions_count = object_count;
                Some(usage)
            }
            Err(e) => {
