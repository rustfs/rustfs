...
impl std::fmt::Display for DnsError {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            DnsError::ClusterDnsFailed { domain, source } => write!(f, "Cluster DNS query failed, falling back to public DNS: {} - {}", domain, source),
            DnsError::PublicDnsFailed { domain, source } => write!(f, "Public DNS resolution failed: {} - {}", domain, source),
            DnsError::AllAttemptsFailed { domain } => write!(f, "All DNS resolution attempts failed for domain: {}. Please check your domain spelling, cluster configuration, or contact an administrator", domain),
    ...
    fn validate_domain_format(domain: &str) -> Result<(), DnsError> {
        ...
        if domain.len() > MAX_FQDN_LENGTH {
            return Err(DnsError::InvalidFormat {
                reason: format!(
                    "FQDN must not exceed {} bytes, got {} bytes", 
                    MAX_FQDN_LENGTH, 
                    domain.len()
                ),
            });
