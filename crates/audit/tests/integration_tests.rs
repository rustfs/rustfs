...
use rustfs_audit::{
    s3_events, AuditConfig, AuditEntry, AuditTargetConfig, DefaultAuditTargetFactory, TargetRegistry,
    AuditSystem, PerformanceConfig, RedactionConfig
};
...
async fn test_concurrent_dispatch_with_failure_isolation() {
    ...
    for entry in entries {
        ...
        tasks.push(tokio::spawn(async move {
            system_clone.log(Arc::new(entry)).await
        }));
    }
