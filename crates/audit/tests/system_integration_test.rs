...
async fn test_global_audit_functions() {
    ...
    let logger = AuditLogger::instance();
...
async fn test_performance_under_load() {
    ...
    for i in 0..100 {
        ...
        let task = tokio::spawn(async move {
            system_clone.dispatch(entry).await
        });
        tasks.push(task);
    }
...
fn create_sample_audit_entry() -> AuditLogEntry {
    create_sample_audit_entry_with_id(0)
}

fn create_sample_audit_entry_with_id(id: u32) -> AuditLogEntry {
    AuditLogEntry {
        version: "1".to_string(),
        ...
        object_name: Some(format!("test-object-{}", id)),
        version_id: None,
        request_path: Some(format!("/test-bucket/test-object-{}", id)),
        request_method: Some("PUT".to_string()),
        request_query: None,
        response_status: Some(200),
        request_header_authorization: None,
        request_header_content_length: Some(1024),
        request_header_content_type: Some("application/octet-stream".to_string()),
        response_header_content_length: Some(0),
        request_remote_addr: Some("127.0.0.1:8080".to_string()),
        request_bytes_received: Some(1024),
        response_bytes_sent: Some(0),
        request_time_to_response_ns: Some(1000000), // 1ms in nanoseconds
        tags: None,
        api_details: None,
    }
}