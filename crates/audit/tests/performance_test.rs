...
async fn test_audit_system_startup_performance() {
    ...
    let result = timeout(Duration::from_secs(5), system.start(config)).await;
...
async fn test_audit_log_dispatch_performance() {
    ...
    let audit_entry = AuditLogEntry {
        ...
        object_name: Some("test-object".to_string()),
        version_id: None,
        request_path: Some("/test-bucket/test-object".to_string()),
