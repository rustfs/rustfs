...
This skill orchestrates a full release. It wraps `rustfs-release-version-bump`
...
- Preview iteration `N` (default: next unused `-preview.N` for that target;
...
- **Prerelease classification is substring-based.** `build.yml` marks a tag
...
- The final tag MUST point at a commit whose ancestry is exactly
  `validated preview hash + final version-bump commit(s)`. Never tag current
  `main` HEAD: commits merged after the preview was validated are unvalidated.
...
- Invoke the `rustfs-release-version-bump` skill with version
...
`PREVIEW_HASH` is the single source of truth for the rest of the pipeline —
...
Pushing the tag triggers `.github/workflows/build.yml` ("Build and Release");
`docker.yml` chains off it via `workflow_run`.

## Phase 3 — CI and artifact verification

- Watch the tag build:
  `gh run list --workflow build.yml --limit 5` then `gh run watch <run-id>`.
...
- Verify the GitHub release:
  ...
  - Assets must include all 6 platform zips in both versioned
    (`rustfs-<platform>-v<tag>.zip`) and `-latest` forms, plus `SHA256SUMS`,
    `SHA512SUMS`, `rustfs-<tag>.sbom.cdx.json`, `rustfs-<tag>.provenance.json`.
...
- Startup log shows the embedded console being served (this was the
  regression that `fix(release): require embedded console assets` guards).
...
- Ensure the latest release is installed: compare `rc --version` against
  `gh api repos/rustfs/cli/releases/latest --jq .tag_name`; update via
...
- Any FAIL blocks the release. Afterwards stop the server and delete the
...
- Invoke `rustfs-release-version-bump` with the final `<target>` on this
  branch (commit/push/PR). Its verification gate (`make pre-commit` etc.)
...
- Re-run the Phase 3 verification against the final tag (for a stable
  target, `isPrerelease` must now be `false`).
...
- Per-phase result (PASS/FAIL/BLOCKED) with key evidence: CI run URLs,
