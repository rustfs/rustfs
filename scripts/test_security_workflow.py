...
class FunctionalWorkflowTests(unittest.TestCase):
    ...
    # Suites following the new (release) gate semantics; see FunctionalEvidenceTests.
    NEW_SEMANTICS = ("table",)
    ...
    def test_failure_and_always_step_wiring(self) -> None:
        for suite, job_id in self.JOBS.items():
            with self.subTest(suite=suite):
                ...
                if suite in self.DIRECT_TESTS and suite not in self.NEW_SEMANTICS:
                    # legacy semantics: the run step must fail the job; new-
                    # semantics suites (table) keep it green via continue-on-error
...
class FunctionalEvidenceTests(WorkflowSteps, unittest.TestCase):
    # "table" follows the new (release) semantics (issue-manager step,
    # green on case failures) that main will adopt at the next release->main
    # sync. The legacy-semantics matrices below skip it via NEW_SEMANTICS;
    # wiring and allowlist matrices cover it through the full SUITES list.
    SUITES = (*FunctionalWorkflowTests.DIRECT_TESTS, "heal", "performance", "table")
    NEW_SEMANTICS = ("table",)
    ...
    def test_evidence_wiring_and_failed_initialization_cannot_publish_stale_files(self):
        for suite, suffix in ((suite, suffix) for suite in self.SUITES for suffix in ("", "-scratch")):
            with self.subTest(suite=suite, collision=suffix or "artifact"):
                ...
                self.assertFalse(Path(self.env["GITHUB_ENV"]).exists())
                if suite == "table":
                    # New (release) semantics: the issue-manager step replaces
                    # the legacy failure filing and must skip cleanly when the
                    # token is absent; stale evidence stays untouched.
    ...
    def test_reports_use_only_current_complete_suite_evidence(self):
        for suite in (s for s in self.SUITES[:-1] if s not in self.NEW_SEMANTICS):
