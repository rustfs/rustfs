...
impl ObjectStore {
    ...
    fn decrypt_data(data: &[u8]) -> Result<Vec<u8>> {
        let de = crypto::decrypt_data(get_global_action_cred().unwrap_or_default().secret_key.as_bytes(), data)?;
        Ok(de)
    }

    fn encrypt_data(data: &[u8]) -> Result<Vec<u8>> {
        let en = crypto::encrypt_data(get_global_action_cred().unwrap_or_default().secret_key.as_bytes(), data)?;
        Ok(en)
    }
