    ...
    pub fn document(self, value: &Document) {
        match value {
            ...
            Document::Timestamp(ts) => self
                .date_time(ts, Format::EpochSeconds)
                .expect("epoch-second timestamp formatting should be infallible"),
            Document::BigInteger(value) => self.write_raw_value(value.as_ref()),
            Document::BigDecimal(value) => self.write_raw_value(value.as_ref()),
            _ => unreachable!("unknown aws_smithy_types::Document variant"),
        }
    }
