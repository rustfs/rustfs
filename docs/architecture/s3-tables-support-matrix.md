...
| Catalog-vended table credentials | Automated when enabled | Disabled by default. LoadTable vends only when `X-Iceberg-Access-Delegation` contains the exact `vended-credentials` token. The dedicated credentials endpoint uses the same issuer path. |
...
|---|---|---|
| PyIceberg | Automated | Creates namespace and table, appends rows, reloads, scans, probes metadata-location, refs, views, maintenance, diagnostics, and optional native LoadTable credential resolution with no long-term S3 data credentials after bootstrap. |
...
| LoadTable and table credentials endpoint | Supported | LoadTable keeps the client-provided credential mode and an empty `storage-credentials` list unless the request contains an exact `vended-credentials` delegation token. Requested vending requires separate credential permission. One temporary session is advertised for both the table warehouse prefix and the exact current metadata location; its policy keeps read/write access within the warehouse and grants only `GetObject` to that metadata object. Responses that may carry credentials set `Cache-Control: no-store, private`, `Pragma: no-cache`, and `Expires: 0`. |
...
| Catalog-vended credentials | Automated when enabled | `rustfs-vended-credentials` uses native PyIceberg `RestCatalog.load_table` resolution, removes catalog and ambient long-term S3 data credentials before the load, then checks `PutObject`, `HeadObject`, `GetObject`, and `DeleteObject` inside the table prefix and denies access outside it. |
