...
| Strong snapshot rolling compatibility | Supported | Durable strong mode reads snapshot versions 1 and 2, writes version 1 by default, and writes version 2 only after both the requested and fleet-confirmed gates are enabled. A running compatible writer that has observed version 2 preserves that high-water mark across later reloads. |
...
9. After any version 2 snapshot is persisted, do not roll catalog writers back
   ...
   even when the gates are later disabled. This high-water mark is process-local:
   restoring a version 1 snapshot and restarting every writer is an explicit
   rollback that cannot be inferred from the restored object alone. Recovery
   must restore a compatible binary and a version 2 snapshot together.
10. A version 1 snapshot that contains the same identifier as both a table and
    ...
    each cleanup mutation must reduce the collision set, and unrelated writes
    remain blocked until all collisions are removed. Drain catalog writers that
    predate cleanup quarantine before starting this repair, and complete cleanup
    before the first version 2 write. Restoring a colliding version 1 snapshot
    after a writer has observed version 2 fails closed instead of downgrading the
    durable snapshot.
