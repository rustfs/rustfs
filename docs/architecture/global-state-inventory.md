...
| `RUNTIME_REGISTRY` | `crates/kms/src/policy.rs` | Process-global owner-local admission registry | KMS policy generations share active capacity by backend identity; bounded queue and circuit-breaker state are shared by backend identity plus policy scope. Callers access it only through `RetryPolicy`. |
