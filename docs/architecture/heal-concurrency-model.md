...
Startup and administrator admission serialize through the same gate while recovery records are inventoried. A structurally invalid, unsupported, or oversized root intent or terminal receipt is retained and fenced by a separate `quarantined-root-heal-{intent|terminal}-<task-id>.json` marker on its owner disk. The marker binds the task ID, record kind, bounded source-prefix length, SHA-256 digest, and oversized state. A quarantined task ID cannot be reused, but its unknown scope does not prevent an independent compensation request with a fresh ID. Unavailable disks, duplicate owners, malformed quarantine markers, and source records that no longer match their marker remain fail-closed and defer root recovery without blocking the existing replacement-recovery path.
