...
- Branch: `overtrue/arch-ecstore-error-rebalance-layout`
...
- Current phase PR: E-026/E-CORE-001 ECStore error and rebalance owner layout.
- Based on: E-025 branch while PR #3930 is pending; rebase onto `origin/main`
  after E-017 through E-025 merge before opening this PR.
- PR type for this branch: `pure-move`.
- Runtime behavior changes: none expected for E-026; production changes are
  module file moves that keep the existing `error` and `rebalance` module
  paths.
- Rust code changes: move ECStore error facade and rebalance facade modules
...
- CI/script changes: reject restoring ECStore root `store.rs`, `set_disk.rs`,
  ...
  `compress.rs`, `rio.rs`, `error.rs`, or `rebalance.rs`;
...
- Docs changes: record the API-136 through API-242 owner facade,
  ...
  rebalance facade modules after E-026.
