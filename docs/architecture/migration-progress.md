...
- Branch: `overtrue/arch-ecstore-runtime-global-layout`
...
- Current phase PR: E-024/E-RUNTIME-001 ECStore runtime global owner layout.
- Based on: E-023 branch while PR #3930 is pending; rebase onto `origin/main`
  after E-017 through E-023 merge before opening this PR.
- PR type for this branch: `pure-move`.
- Runtime behavior changes: none expected for E-024; production changes are
  module file moves that keep the existing `global` and `runtime_sources`
  module paths.
- Rust code changes: move ECStore runtime global state and runtime-source
  adapter modules under the `runtime/` owner directory without function-body
  changes.
- CI/script changes: reject restoring ECStore root `store.rs`, `set_disk.rs`,
  ...
  `data_movement_backpressure.rs`, `admin_server_info.rs`, `data_usage.rs`, or
  `get_diagnostics.rs`, `global.rs`, or `runtime_sources.rs`;
...
- Docs changes: record the API-136 through API-242 owner facade,
  ...
  root runtime global modules after E-024.
