...
- Branch: `overtrue/arch-ecstore-core-store-layout-shims`
...
- Current phase PR: E-032 ECStore core/store/layout root shim cleanup.
- Based on: E-031 branch while PR #3932 and follow-up ECStore owner PRs are
  pending; rebase onto `origin/main` after E-019 through E-031 merge before
  opening this PR.
- PR type for this branch: `pure-move`.
- Runtime behavior changes: none expected for E-032; production changes only
  route crate-internal core, store, layout, and data movement ownership through
  owner `mod.rs` files instead of root `lib.rs` path shims.
- Rust code changes: add owner module roots for ECStore core, store, layout, and
  data movement support, then migrate crate-internal imports to those owner paths
  without function-body changes.
...
- Docs changes: record the API-136 through API-242 owner facade,
  ...
  data movement modules after E-032.
