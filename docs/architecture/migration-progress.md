...
- Branch: `overtrue/arch-global-core-runtime-consumer-batch`
...
- Current baseline also includes API-255 from PR #3923, API-256 from PR
  ...
  GLOB-007 storage/server runtime facade consumer batch from PR #3946, and the
  GLOB-007 app runtime facade consumer batch from PR #3947.
- Current phase PR: GLOB-007 core runtime facade consumer batch.
- Based on: `origin/main` after PR #3947 merged.
- PR type for this branch: `ci-gate`.
- Runtime behavior changes: none intended.
- Rust code changes: expose root runtime facade aliases for core consumers and
  route auth, init, startup, protocols, config info, and workload admission
  callers through `current_*` names while keeping the underlying root resolvers
  and fallback behavior unchanged.
- CI/script changes: none intended.
- Docs changes: update this progress ledger for the core runtime facade
  consumer batch.
