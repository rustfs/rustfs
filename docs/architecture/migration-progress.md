...
- Branch: `overtrue/arch-ecstore-root-owner-facade-shrink`
...
- Current phase PR: E-031 ECStore root owner facade shrink.
- Based on: E-029/E-030 branch while PR #3931 is pending; rebase onto
  `origin/main` after E-019 through E-030 merge before opening this PR.
- PR type for this branch: `pure-move`.
- Runtime behavior changes: none expected for E-031; production changes only
  route crate-internal diagnostics, services, I/O support, and runtime module
  ownership through owner `mod.rs` files instead of root `lib.rs` path shims.
- Rust code changes: add owner module roots for ECStore diagnostics, services,
  I/O support, and runtime support, then migrate crate-internal imports to
  those owner paths without function-body changes.
- CI/script changes: reject restoring ECStore root `store.rs`, `set_disk.rs`,
  ...
  reject restoring old bucket/set-disk metadata owner paths, and reject
  restoring ECStore root `lib.rs` owner path shims for diagnostics, services,
  I/O support, and runtime modules;
...
- Docs changes: record the API-136 through API-242 owner facade,
  ...
  owner paths after E-030, and reject restoring ECStore root owner path shims
  for diagnostics, services, I/O support, and runtime modules after E-031.
