...
- Branch: `overtrue/arch-ecstore-erasure-owner-layout`
...
- Current phase PR: E-033/E-004 ECStore erasure owner layout.
- Based on: E-032 branch while PR #3932 and follow-up ECStore owner PRs are
  pending; rebase onto `origin/main` after E-019 through E-032 merge before
  opening this PR.
- PR type for this branch: `pure-move`.
- Runtime behavior changes: none expected for E-033/E-004; production changes
  only route ECStore erasure codec and erasure coding ownership through the
  `erasure` owner module.
- Rust code changes: move ECStore erasure codec and erasure coding modules under
  `crates/ecstore/src/erasure`, then migrate crate-internal imports to owner
  paths without function-body changes.
- CI/script changes: reject restoring ECStore root `store.rs`, `set_disk.rs`,
  ...
  restoring ECStore root RPC support files, keep the ECStore root RPC module as a
  re-export-only facade, reject restoring old bucket/set-disk metadata owner
  paths, and reject
...
- Docs changes: record the API-136 through API-242 owner facade,
  ...
  data movement modules after E-032, and reject restoring ECStore root erasure
  codec or coding modules after E-033/E-004.
