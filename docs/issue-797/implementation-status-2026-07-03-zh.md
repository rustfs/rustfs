...
| Task 08 batch read-version RPC | 新增 `BatchReadVersion` RPC、payload decode 与 fallback | `off` | `b48daeee5` | `cargo test -p rustfs-ecstore batch_read_version --lib`; `cargo test -p rustfs --lib batch_read_version` |
...
- `RUSTFS_METADATA_BATCH_READ=on` 遇到 unsupported 节点时必须显式失败，不能静默降级。
