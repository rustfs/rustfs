...
| Task 08 batch read-version RPC | 新增 `BatchReadVersion` RPC、payload decode 与 fallback；`auto` 才对旧节点 fallback，`on` 对 unsupported 显式失败；代码读取文档主开关 `RUSTFS_METADATA_BATCH_READ`，并兼容旧别名 `RUSTFS_BATCH_METADATA_RPC`；单次调用只解析一次 mode，避免请求内开关语义漂移；RPC start/fallback/no-fallback 日志携带 mode；新增 `rustfs_remote_disk_batch_read_version_gate_total{mode,decision}` 低基数 counter | `off` | `b48daeee5`, `c70fa8397`, `d7dff3ea1`, `e2a778e7b`, `375b70cb4`, current follow-up | `cargo test -p rustfs-ecstore batch_metadata_rpc_mode --lib`; `cargo test -p rustfs-ecstore batch_read_version --lib`; `cargo test -p rustfs --lib batch_read_version` |
...
`RUSTFS_METADATA_BATCH_READ` 是 Task 08 的主开关名。代码仍兼容旧别名 `RUSTFS_BATCH_METADATA_RPC`，但当两个变量同时存在时，以 `RUSTFS_METADATA_BATCH_READ` 为准。
...
- Batch read-version gate decision：`rustfs_remote_disk_batch_read_version_gate_total{mode,decision}`。
