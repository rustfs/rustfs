...
| PR, non-doc change | `Test and Lint (rio-v2)`, `Test and Lint (swift)`, `Test and Lint (sftp)`, `Test and Lint (hash-md5-fast-md5)` | `ci.yml` `test-and-lint-rio-v2`, `test-and-lint-protocols` | Via aggregate | `cargo nextest run` with the job's `--features` on the job's package set (`hash-md5-fast-md5` covers `rustfs-utils` + `rustfs-rio`: the alternate MD5 backend and its consumers) |
