...
async fn broadcast_site_replication_delete_bucket(runtime: &SiteReplicationRuntime, path: &str) -> S3Result<()> {
    let sends = runtime.state.peers.values().filter_map(|peer| {
