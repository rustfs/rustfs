...
mod linux_impl {
    ...
    use rustfs_config::{
        DEFAULT_CPU_DURATION_SECS, DEFAULT_CPU_FREQ, DEFAULT_CPU_INTERVAL_SECS, DEFAULT_CPU_MODE,
        DEFAULT_ENABLE_PROFILING, DEFAULT_MEM_INTERVAL_SECS, DEFAULT_MEM_PERIODIC,
        DEFAULT_OUTPUT_DIR, ENV_CPU_DURATION_SECS, ENV_CPU_FREQ, ENV_CPU_INTERVAL_SECS,
        ENV_CPU_MODE, ENV_ENABLE_PROFILING, ENV_MEM_INTERVAL_SECS, ENV_MEM_PERIODIC, ENV_OUTPUT_DIR,
    };
    ...
    fn output_dir() -> PathBuf {
        ...
        if let Err(e) = create_dir_all(&p) {
            warn!(
                "profiling: create output dir {} failed: {}, fallback to current dir",
                p.display(),
                e
            );
            return PathBuf::from(".");
    ...
    fn read_cpu_mode() -> CpuMode {
        match get_env_str(ENV_CPU_MODE, DEFAULT_CPU_MODE)
            .to_lowercase()
            .as_str()
        {
    ...
    fn write_pprof_report_pb(report: &pprof::Report, path: &Path) -> Result<(), String> {
        let profile = report
            .pprof()
            .map_err(|e| format!("pprof() failed: {e}"))?;
    ...
    async fn dump_cpu_with_guard(guard: &pprof::ProfilerGuard<'_>) -> Result<PathBuf, String> {
        let report = guard
            .report()
            .build()
            .map_err(|e| format!("build report failed: {e}"))?;
    ...
    pub async fn dump_cpu_pprof_for(duration: Duration) -> Result<PathBuf, String> {
        ...
        let guard = pprof::ProfilerGuard::new(freq)
            .map_err(|e| format!("create profiler failed: {e}"))?;
    ...
    pub async fn dump_memory_pprof_now() -> Result<PathBuf, String> {
        ...
        let bytes = prof_ctl
            .dump_pprof()
            .map_err(|e| format!("dump pprof failed: {e}"))?;
        f.write_all(&bytes)
            .map_err(|e| format!("write file failed: {e}"))?;
    ...
    pub async fn init_from_env() {
        ...
        let cpu_interval =
            Duration::from_secs(get_env_u64(ENV_CPU_INTERVAL_SECS, DEFAULT_CPU_INTERVAL_SECS));
        let cpu_duration =
            Duration::from_secs(get_env_u64(ENV_CPU_DURATION_SECS, DEFAULT_CPU_DURATION_SECS));
        ...
        let mem_interval =
            Duration::from_secs(get_env_u64(ENV_MEM_INTERVAL_SECS, DEFAULT_MEM_INTERVAL_SECS));
        if mem_periodic {
