...
pub fn get_s3_equivalent_operation(unsupported_feature: &str) -> Option<&'static str> {
    match unsupported_feature {
        ...
        "APPE command (file append)" | "SSH_FXP_OPEN append mode" => {
            Some("Use PutObject to overwrite the entire object")
        }
        "chmod command" | "chown command" | "SSH_FXP_SETSTAT permission modification" | "SSH_FXP_FSETSTAT permission modification" => {
            Some("Use S3 ACLs or Bucket Policies to manage permissions")
        }
        "SYMLINK creation" | "SSH_FXP_SYMLINK creation" => {
            Some("S3 object storage does not support symbolic links")
        }
