...
impl Objects {
    ...
    pub async fn post_object(&self, req: S3Request<PostObjectInput>) -> S3Result<S3Response<PostObjectOutput>> {
        ...
        {
            return self.post_object_extract(req).await;
        }
            ...
            && let Some(metadata_sys) = rustfs_ecstore::bucket::metadata_sys::GLOBAL_BucketMetadataSys.get()
        {
            ...
            match quota_checker
                .check_quota(&bucket, QuotaOperation::PostObject, size as u64)
                .await
            {
                Ok(check_result) => {
        ...
        // If it is a POST Policy(multipart/form-data) path, the PostObjectInput carries the success_action_* field
        ...
        if success_action_status.is_some() || success_action_redirect.is_some() {
            ...
            // Keep helper event complete (note: (StatusCode, Body) is returned here instead of PostObjectOutput)
            let result = Ok(resp);
    ...
    pub(crate) async fn post_object_extract(&self, req: S3Request<PostObjectInput>) -> S3Result<S3Response<PostObjectOutput>> {
        let helper = OperationHelper::new(&req, EventName::ObjectCreatedPut, "s3:PostObject").suppress_event();
        let input = req.input;
