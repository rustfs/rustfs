...
use handlers::{
    ...
    prometheus::{
        BucketMetricsHandler, ClusterMetricsHandler, NodeMetricsHandler, PrometheusConfigHandler, PrometheusTokenHandler,
        ResourceMetricsHandler,
    },
    rebalance,
...
pub fn make_admin_route(console_enabled: bool) -> std::io::Result<impl S3Route> {
    ...
    r.insert(
        Method::POST,
        "/rustfs/admin/v3/prometheus/token",
        AdminOperation(&PrometheusTokenHandler {}),
    )?;
