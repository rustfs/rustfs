...
mod tests {
    ...
    fn query_defaults_clamps_and_refuses_unknown_parameters() {
        ...
        for (query, message) in [
            ...
            let error = parse_inventory_query(Some(query)).expect_err("invalid query must be refused");
            assert_eq!(error.code(), &S3ErrorCode::InvalidArgument, "{query}");
            assert_eq!(error.message(), Some(message), "{query}");
        }
    }
